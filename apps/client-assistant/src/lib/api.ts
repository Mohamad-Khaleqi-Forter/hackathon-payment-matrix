...
export const api = {
  ...
  sendMessage: async (sessionId: string, message: string, email: string, autoBuy: boolean = false): Promise<ChatResponse> => {
    const response = await fetch(`${API_URL}/chat/${sessionId}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ text: message, email, autoBuy }),
    });
